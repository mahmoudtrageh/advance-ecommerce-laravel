...
class IndexController extends Controller
{
    public function index(){
    	...
    	return view('frontend.index',compact('categories','sliders','products','featured','hot_deals','special_offer','special_deals','skip_category_0','skip_product_0','skip_category_1','skip_product_1','skip_brand_1','skip_brand_product_1','blogpost', 'brands'));
  ...
  // Subcategory wise data
   ...
   $list_view = view('frontend.product.list_view_product',compact('products'))->render();
	return response()->json(['grid_view' => $grid_view,'list_view',$list_view]);	
		...
		return view('frontend.product.subcategory_view',compact('products','categories','breadsubcat','subcat_id', 'slug'));

	}

	public function TagShopFilter (Request $request,$tag){
		$minPrice = (int) explode(",",$request->price)[0];
		$maxPrice = (int) explode(",",$request->price)[1];
	   
		$products = Product::select("*")->where('product_tags_en', $tag)->whereBetween('discount_price', [$minPrice, $maxPrice])->paginate();
		$categories = Category::orderBy('category_name_en','ASC')->get();

		return view('frontend.tags.tags_view',compact('products','categories', 'tag'));
	}

	public function subCatShopFilter (Request $request, $subcat_id,$slug){
		$minPrice = (int) explode(",",$request->price)[0];
		$maxPrice = (int) explode(",",$request->price)[1];
	   
		$products = Product::select("*")->where('subcategory_id',$subcat_id)->whereBetween('discount_price', [$minPrice, $maxPrice])->paginate();
		$categories = Category::orderBy('category_name_en','ASC')->get();
	...
	public function subsubCatShopFilter (Request $request, $subsubcat_id,$slug){
		$minPrice = (int) explode(",",$request->price)[0];
		$maxPrice = (int) explode(",",$request->price)[1];
	   
		$products = Product::select("*")->where('subsubcategory_id',$subsubcat_id)->whereBetween('discount_price', [$minPrice, $maxPrice])->paginate();
		$categories = Category::orderBy('category_name_en','ASC')->get();

		$breadsubsubcat  = SubSubCategory::with(['category'])->where('id',$subsubcat_id)->get();

		return view('frontend.product.sub_subcategory_view',compact('products','categories','breadsubsubcat','subsubcat_id', 'slug'));
	}
 ...
 // Product Seach 
	public function ProductSearch(Request $request){
		...
		$products = Product::where('category_id',$category_id)->where('product_name_hin','LIKE',"%$item%")->get();
		} else {
			$products = Product::where('category_id',$category_id)->where('product_name_en','LIKE',"%$item%")->get();
